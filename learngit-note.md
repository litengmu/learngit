...
****
## Git简介
* Git是分布式版本控制系统，由linus在2005年用C语言开发的。
...
```
**注意**  可以多次`git add `，添加多个文件；最后一次`git commit`
...
`commit-id`可以用过 `git log`查看 ，  只要输入前几位就可以了
...
工作区就是电脑中能看到的目录，如学习中用到的`learngit`，工作区有一个隐藏目录`.git`，这是Git的版本库 。
...
```
回到场景1，再按场景1操作
* 场景3：已经commit但没有推送到远程库——**版本回退**，前提是没有推送到**远程库**。
## 删除文件
...
1.如果你用的`rm file`删除文件，那就相当于只删除了工作区的文件，如果想要恢复，直接用`git checkout -- <file>`就可以
2.如果你用的是`git rm file `删除文件，那就相当于不仅删除了文件，而且还添加到了暂存区，需要先`git reset HEAD <file>`（版本回退），然后再`git checkout -- <file>`
